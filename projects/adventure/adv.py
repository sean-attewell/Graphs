...
#     back_steps = -1
#     last_move = traversalPathCopy[back_steps]
#     q.enqueue(last_move)
...
#                 for value in traversal_graph[key]:
#                     if traversal_graph[key][value] == "?":
#                         noUnexplored = False
#                         break
#                 if noUnexplored == False:
#                     break
#             if noUnexplored:
#                 completed = True
#                 return completed
...
#             else:
#                 back_steps -= 1
#                 q.enqueue(traversalPathCopy[back_steps])
...
#         completed = bfs(player.currentRoom.id, "?")
#     print("All explored")
...
def dft(player):
    ...
    while s.size() > 0:
        ...
        if current_room.id not in visited:
            ...
            # keep track of stack size to see if a new room was added
            stack_before = len(s.stack)

            obj = {'n': None, 'w': None, 's': None, 'e': None}

            # record which room is in each direction
            for direction in current_room.getExits():
                # get room in direction
                room = current_room.getRoomInDirection(direction)
                obj[direction] = room

            # make list of tuples of directions which do have a room
            obj_no_none = [i for i in obj.items() if i[1] != None]

            # add the neighbouring room with the last possible paths 
            # to the stack last (so it will be processed first)
            # this will prevent long paths back
            # and make sure dead ends rooms are delt with first

            # sort the list based on length of exits each connected room has
            # the room with least amount of exits will be added to stack last and visited first
            
            sorted_obj = sorted(obj_no_none, key=lambda room: len(room[1].getExits()), reverse=True)
            ...
            if stack_before == stack_after:
                ...
                try:
                    ...
                    if current_room.id == s.stack[-1].id:
                         shortest_path = bfs(current_room, s.stack[-2])

                    else:
                        shortest_path = bfs(current_room, s.stack[-1])
